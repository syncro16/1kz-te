...
volatile uint8_t enrichment = 100;
...
void setup() {
	...
	attachInterrupt(0, rpmTrigger, CHANGE);  // 0 == Input #2 on arduino uno
...
void rpmTrigger() {
	...
	if (state == 0) {
		...
		if (enrichment) {
			...
			uint32_t adjust = ((uint32_t)injectionDuration<<8)/(256-enrichment);
...
void loop() {
	...
	enrichment = map(val,406,819,0,255); 	
	
	// generate reference pulse to pin13 (led). 
	// This is 1000rpm (16.66r/s = 60ms) - if there is one trigger per 360° rotation
	// Because there is 4 triggers per rotation, and pump rotates two times faster than engine
